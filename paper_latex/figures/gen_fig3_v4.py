...
Version 4: Final polished version with clean label spacing.
"""
...
for idx, (dset, metric) in enumerate(zip(datasets, metric_types)):
    ...
    if dset == 'ZINC-subset':
        ...
        val_fmt = '.3f'
        std_fmt = '.3f'
        label_gap = 0.012  # gap above error bar to value label
        std_gap = 0.008    # gap between value label and std label
    ...
    for i, (bar, val, err) in enumerate(zip(bars, vals, errs)):
        ...
        top = val + err + label_gap

        # Main value
        fw = 'bold' if i == best_idx else 'semibold'
        fs = 10.5 if i == best_idx else 9.5
        ax.text(cx, top, f'{val:{val_fmt}}',
                ha='center', va='bottom', fontsize=fs, fontweight=fw,
                color='#222222')

        # Std deviation below the value (lighter, smaller)
        ax.text(cx, top - std_gap, f'±{err:{std_fmt}}',
    ...
    ax.text(bx, sy, '★', ha='center', va='center',
            fontsize=16, color='#DAA520')
